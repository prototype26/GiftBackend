...
function test() {
    ...
    data.forEach(json => {
        ...
        if (json.type == "category") {
            // var arr = [];
            // obj.giftCardsMainCategory = json.attributes.name;
            json.attributes.sub_categories.forEach(subcatJson => {
                var obj = {
                    giftCardsMainCategory:"",
                    giftCardsSubCategory:"",
                }
               obj.giftCardsMainCategory = json.attributes.name;
               obj.giftCardsSubCategory = ""; 
               obj.giftCardsSubCategory = subcatJson.code;
               cardsArr.push(obj);
            });
            // obj.giftCardsSubCategory = arr;
        }
        
    });
    console.log(`cardsArr: ${JSON.stringify(cardsArr)}`);
    axios.post('http://localhost:3002/giftapi/v1/giftsCards', cardsArr)
       .then(response => {
